...
public class Ball 
{
	 ...
	 */
	public Ball(double x, double y, double diameter, String col, double xSpeed, double ySpeed)
	{
		this.xPosition = x;
		this.yPosition = y;
		this.size = diameter;
	...
	public void play(GameArena arena){
		this.checkXPosition(arena);
		this.checkYPosition(arena);
		this.moveHorizontally(xSpeed);
		this.moveVertically(ySpeed);

	}

	public static Ball[] makeBalls(int amountOfBalls){
		Random rand = new Random();

		Ball[] balls = new Ball[amountOfBalls];
		for(int i = 0; i < balls.length; i++){
			balls[i] = new Ball(rand.nextDouble() * 500, rand.nextDouble() * 200 + 5, 6, "BLUE", rand.nextDouble() * 5 + 2, 0);
		}
		return balls;
	}

	public void checkXPosition(GameArena arena) {
		if (xPosition >= arena.getArenaWidth() - size / 2 || xPosition <= size / 2) {
			xSpeed = xSpeed * -1;
		}
		else {
			xSpeed = xSpeed * 0.995;
	...
	public void checkYPosition(GameArena arena) {
		if (yPosition >= arena.getArenaHeight() - size / 2 || yPosition <= size / 2) {
	...
	// public void playWithBalls(GameArena arena, Ball[] balls, int xSpeed, int ySpeed){
